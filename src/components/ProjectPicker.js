    ...
    return (
          <div className="relative flex flex-col w-3/4 m-auto top-24">
            ...
            <div className="flex flex-col flex-no-wrap items-start my-3 overflow-x-scroll scrolling-touch md:items-center md:flex-row new-scrollbar">
              {projects.map((proj) => {
                 return (
                    <div className="flex-none w-64 py-12 m-5 4k:w-80 md:w-72 h-72 4k:h-fit card">
                      <p className="block h-16 font-bold 4k:h-32 4k:text-4xl">{proj.name}</p>
                      <ul className="h-32 4k:h-64 text-balance 4k:text-2xl">
                        <li className="w-5/6 m-auto">{proj.description}</li>
                      </ul>
                      {proj.deployed && 
                        <div className="inline-block 4k:text-2xl">
                          <svg className="inline-block" data-testid="geist-icon" height="16" stroke-linejoin="round" viewBox="0 0 16 16" width="16" style={{color: "black"}}>
